...
public:
    ...
    static void visualize_marker(int marker_id,const cv::Mat T,float marker_size,pcl::PointCloud<pcl::PointXYZRGB>& point_cloud);
    static void visualize_markers(const std::vector<cv::Mat> &transforms_to_root_marker, const std::vector<int> index2id, float marker_size, pcl::PointCloud<pcl::PointXYZRGB> &point_cloud_markers, cv::Mat T);

    void visualize_cameras(pcl::PointCloud<pcl::PointXYZRGB> &point_cloud_cameras, float Z=0.1);
    void visualize_markers(pcl::PointCloud<pcl::PointXYZRGB> &point_cloud_markers);
#endif
