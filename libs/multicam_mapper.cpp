...
void MultiCamMapper::write_detections_file(string path, vector<vector<vector<aruco::Marker>>> &input){
    ...
    for(size_t frame_num=0;frame_num<input.size();frame_num++){
        ...
        for(size_t cam=0;cam<frame_markers.size();cam++){//loop over all possible cameras
            ...
            for(int m=0;m<num_cam_markers;m++)
                MultiCamMapper::serialize_marker(frame_markers[cam][m],detections_file);
...
void MultiCamMapper::serialize_marker(aruco::Marker marker, ostream& output){
...
void MultiCamMapper::deserialize_marker(istream& input, aruco::Marker& marker){
...
void MultiCamMapper::project_marker(const MatArrays &ma, size_t frame_id, size_t marker_id, size_t cam_id, vector<cv::Point2f> &points_2d){
    ...
    vector<cv::Point3f> points_3d=aruco::Marker::get3DPoints(marker_size);
    cv::Mat rvec;
    cv::Rodrigues(transform(cv::Range(0,3),cv::Range(0,3)),rvec);
    cv::Mat tvec=transform(cv::Range(0,3),cv::Range(3,4));

    cv::projectPoints(points_3d,rvec,tvec,cam_mat,dist_coeffs,points_2d);
}
    ...
    for(auto frame_it=frame_cam_markers.begin();frame_it!=frame_cam_markers.end();frame_it++){
        ...
        for(auto cam_it=cam_markers.begin();cam_it!=cam_markers.end();cam_it++){
            ...
            for(size_t m=0;m<num_m;m++)
                serialize_marker(markers[m],ofile);
        }
    }
}
...
void MultiCamMapper::deserialize_frame_cam_markers(ifstream &ifile){
    ...
    for(size_t f=0;f<num_f;f++){
        ...
        for(size_t c=0;c<num_c;c++){
            ...
            for(size_t m=0;m<num_m;m++)
                deserialize_marker(ifile,frame_cam_markers[f][c][m]);
        }
    }
}
...
bool MultiCamMapper::read_solution_file(string path){
    ...
    //prepare the transforms to root
...
//    auto markers=frame_cam_markers[frame_id][camera_id];
    ...
    if(mat_arrays.object_to_global.m.find(frame_id)==mat_arrays.object_to_global.m.end())
        return;

    for(auto it=marker_cam_frame.begin();it!=marker_cam_frame.end();it++){
        int marker_id=it->first;
        vector<cv::Point2f> points_2d;
        project_marker(mat_arrays,frame_id,marker_id,camera_id,points_2d);
        for(int j=0;j<points_2d.size();j++)
...
void MultiCamMapper::write_text_solution_file(string text_path){
    ...
    for(std::pair<const int,int> &p : mat_arrays.transforms_to_root_marker.m){
        ...
        fs<<"transform"<<mat_arrays.transforms_to_root_cam.v[marker_index];
        fs<<"}";
    }
    fs<<"]";
...
void MultiCamMapper::visualize_sequence(string path, size_t num_total_frames){
    ...
    for(int id=0;id<num_total_frames;id++){
        if(mat_arrays.object_to_global.m.find(id) != mat_arrays.object_to_global.m.end()){
            ...
            visualize_markers(mat_arrays.transforms_to_root_marker.v,mat_arrays.transforms_to_root_marker.id,marker_size,*tmp_output,mat_arrays.object_to_global.v[index]);
...
void MultiCamMapper::draw_3d_line_points(cv::Mat start, cv::Mat end, int steps, cv::Scalar start_color, pcl::PointCloud<pcl::PointXYZRGB>& point_cloud, cv::Scalar end_color){
    ...
    for(int i=0;i<steps;i++,point+=step,color+=color_step){
        pcl::PointXYZRGB p(start_color[0],start_color[1],start_color[2]);
        p.x=x;
        p.y=y;
        p.z=z;
...
void MultiCamMapper::visualize_camera(int cam_num, const cv::Mat cam_mat, cv::Mat T_in, const cv::Size im_size,double Z, pcl::PointCloud<pcl::PointXYZRGB>& point_cloud){
...
void MultiCamMapper::visualize_cameras(const std::vector<CamConfig> &cam_confs, const vector<cv::Mat> &transforms_to_root_cam, const vector<int> &index2id, float Z, pcl::PointCloud<pcl::PointXYZRGB>& point_cloud_cameras){
...
void MultiCamMapper::visualize_cameras(pcl::PointCloud<pcl::PointXYZRGB>& point_cloud_cameras, float Z){
    for(size_t i=0;i<mat_arrays.transforms_to_root_cam.v.size();i++)
        visualize_camera(mat_arrays.cam_mats.id[i],mat_arrays.cam_mats[i],mat_arrays.transforms_to_root_cam[i],image_sizes[i],Z,point_cloud_cameras);
}

void MultiCamMapper::visualize_marker(int marker_id,const cv::Mat T_in,float marker_size,pcl::PointCloud<pcl::PointXYZRGB>& point_cloud){
    cv::Mat T;
    T_in.convertTo(T,CV_32FC1);
...
void MultiCamMapper::visualize_markers(const vector<cv::Mat> &transforms_to_root_marker, const vector<int> index2id, float marker_size, pcl::PointCloud<pcl::PointXYZRGB> &point_cloud_markers, cv::Mat T){
...
void MultiCamMapper::visualize_markers(pcl::PointCloud<pcl::PointXYZRGB> &point_cloud_markers){
    MultiCamMapper::visualize_markers(mat_arrays.transforms_to_root_marker.v,mat_arrays.transforms_to_root_marker.id, marker_size, point_cloud_markers, cv::Mat::eye(4,4,CV_64FC1));
}
#endif
